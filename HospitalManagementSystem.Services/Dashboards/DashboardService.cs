    ...
    public class DashboardService : IDashboardData
    {
        ...
        public async Task<object> GetDashboardData()
        {
            ...
            try
            {
                ...
                var appointmentTodayCounts = await _dbContext.AppointmentMaster.Where(x => x.AppointmentDate.Date == DateTime.Now.Date).CountAsync();
                var medicineTodayCounts = await _dbContext.MedicineMaster.Where(x => x.CreatedDate.Date == DateTime.Now.Date).CountAsync();
                var medicineTotalCounts = await _dbContext.MedicineMaster.CountAsync();
                var patientsTodayCounts = await _dbContext.PatientMaster.Where(x => x.CreatedDate.Date == DateTime.Now.Date).CountAsync();
                    ...
                    .Join(_dbContext.PatientMaster,
                        ...
                        (AM, PM) => new
                        {
                            ...
                            AM.CreatedDate
                                    ...
                                    x => x.AppointmentDate.Date == DateTime.Now.Date).ToListAsync();

                var todayPatientDetails = await _dbContext.PatientMaster.Where(x => x.CreatedDate.Date == DateTime.Now.Date).ToListAsync();
                ...
                var cardAmount = _dbContext.PrescriptionBillings
                                .Where(x => x.PaymentType == PaymentConst.Card && (x.CreatedDate.Date == DateTime.Now.Date)).Sum(x => x.PaidAmount);

                var upiAmount = _dbContext.PrescriptionBillings
                                .Where(x => x.PaymentType == PaymentConst.UPI && (x.CreatedDate.Date == DateTime.Now.Date)).Sum(x => x.PaidAmount);

                var otherAmount = _dbContext.PrescriptionBillings
                                .Where(x => x.PaymentType == PaymentConst.Other && (x.CreatedDate.Date == DateTime.Now.Date)).Sum(x => x.PaidAmount);
